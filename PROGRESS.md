...
### HAL implementations
- `libs/hal/esp32s3/` — Pocket implementations of all 7 interfaces
...
### UI layer (`libs/ui/`)
- LVGL screen stack (`ui::ScreenStack`)
- Status bar widget (mode, WPM, battery, WiFi/BT icons)
- Main menu screen
- Per-mode screens (keyer, generator, trainer, decoder, transceiver, prefs)

### Content extensions (`libs/content/`)
- QSO phrase engine (`qso_phrase.h/.cpp`) with template + random pool substitution
- Full QSO simulation sequencer for echo trainer
...
### Unit tests
- `test/native/` — tests for cw-engine, cw-decoder, content, training (all native-compilable)

### `src/main.cpp`
- Refactor from current LVGL demo stub into proper HAL wiring + task startup
